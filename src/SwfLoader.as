package 
{
	 ...
	 * 素材加载器
	 * ...
	 my dog died when it was 13 years old
	 * @author db0@qq.com
	 */
	public class SwfLoader 
	{
		...
		public static function loadData(url:String,loaded:Function=null,type:String=null,urlVariables:URLVariables=null):URLLoader
		{
		 ...
		 * 根据文件内容排序文件
		 文件内容就是当前目录下的文件
		 内容可以用 ls > file.txt 生成
		 */
		public static function sortBytxtFile(arr:Array,txtFile:String="file.txt"):Array
		{
			if(arr && arr.length >0){
				var dir:String = arr[0] as String;
				if(dir){
					dir = dir.replace(/^(.*)[\\\/]+[^\\\/]+[\\\/]*$/,"$1");
					var fs:String = readfile(dir+"/"+txtFile);
					if(fs == null)return arr;
					fs = fs.replace(/^[\ufe\uff\uef\ubb\ubf]*/i,"");//txt 文件头 
					fs = fs.replace(/^[^<]*/i,"");
					var xml:XML = XML(fs);
					fs = xml.toString();
					logs.adds("sortBytxtFile dir:",dir);
					if(fs){
						fs = fs.replace(/^[\r\n]+/ig,"");
						fs = fs.replace(/[\r\n]+$/ig,"");
						fs = fs.replace(/[\r\n]+/ig,"\n");
						var farr:Array = fs.split("\n");
						logs.adds("sortBytxtFile content Array:",farr);

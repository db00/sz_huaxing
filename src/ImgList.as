	...
	[SWF(width="1024",height="768",frameRate="12",backgroundColor=0x000000)]
		...
		public class ImgList extends Sprite
		{
			...
			private function show_list(e:Event):void
			{
				...
				else if (e.type == "hidelist")
				{
					haslist = false;
				}
				select_index = imglistshow.index;
				dispatchEvent(e);
			}
...
_bgbmp:图标背景
			...
			private function addbig():void
			{
				if (hasbig)
				{
					...
					var rect:Rectangle = new Rectangle((1 - xrate) / 2 * w, 0, w * xrate, h);
					...
					/*
					   ...
					   prev_btn.alpha = next_btn.alpha = 0;
					 */
					xrate=.7;w=h=800;rect= new Rectangle((1 - xrate) / 2 * w, 0, w * xrate, h);//大事记
					imglistshow = new ImgListShow(rect, close_btn, next_btn, prev_btn);
					if (imglistshow.parent == null)
					...
					imglistshow.addEventListener(Event.CHANGE, show_list);
				}
			}

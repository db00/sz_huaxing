...
package 
{
	...
	public class Videos extends Sprite
	{
		 ...
		 * @param	_height
		 */
		public function setSize(_x:int = 0, _y:int = 0, _width:int = 6480, _height:int = 1920):void
		{
			//bg.visible = true;
			//bg.width = _width;
			//bg.height = _height;
			...
			if(bg==null){
				bg = new Bitmap(new BitmapData(_width,_height,false,0xff000000));
				visible = false;
				addChildAt(bg,0);
			}
		}
		...
		private function start(vol:Number):void
		{
			...
			nc.addEventListener(SecurityErrorEvent.SECURITY_ERROR, ioError);
			ns.addEventListener(IOErrorEvent.IO_ERROR, ioError);
			ns.addEventListener(AsyncErrorEvent.ASYNC_ERROR, ioError);
		...
		private function ioError(e:Event=null):void
		{
			visible = false;
		}
